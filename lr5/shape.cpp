...
void Shape::setId(int id) { m_id = id; }
QString Shape::getName() const { return m_name; }
void Shape::setName(const QString& name) { m_name = name; }
...
void Shape::setVisible(bool visible) { m_isVisible = visible; }
bool Shape::hasFill() const { return m_fill; }
void Shape::setFill(bool fill) { m_fill = fill; }
QColor Shape::getFillColor() const { return m_fillColor; }
void Shape::setFillColor(const QColor& color) { m_fillColor = color; }
QPoint Shape::getAnchorPoint() const { return m_anchorPoint; }
void Shape::setAnchorPoint(const QPoint& point) { m_anchorPoint = point; }
...
void Shape::moveAnchorPoint(const QPoint& newPoint)
{
...
int Shape::getHandleCount() const { return m_resizeHandles.size(); }

int Shape::hitTestResizeHandle(const QPoint& point)
{
    updateResizeHandles();
...
bool Shape::hitTestAnchorHandle(const QPoint& point)
{
    updateAnchorHandle();
    return m_anchorHandle.contains(point);
}

void Shape::drawResizeHandles(QPainter& painter)
{
...
void Shape::drawAnchorHandle(QPainter& painter)
{
    if (!m_isSelected) return;
    updateAnchorHandle();
...
void Shape::setSideColor(int index, const QColor& color)
{
...
void Shape::setSideWidth(int index, int width)
{
...
QColor Shape::getSideColor(int index) const
{
    return (index >= 0 && index < edgeColors.size()) ? edgeColors[index] : Qt::black;
}

int Shape::getSideWidth(int index) const
{
...
bool Shape::fromJson(const QJsonObject& json)
{
    m_id = json["id"].toInt();
...
void Shape::updateResizeHandles()
{
...
QPointF Shape::calculateMiterJoin(const QPointF& p1, const QPointF& p2, const QPointF& p3,
...
QPointF Shape::lineIntersection(const QPointF& p1, const QPointF& p2,
